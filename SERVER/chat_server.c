...
char message[100] = "";		//This array will store the messages that are send by the server


serv.sin_family = AF_INET;
serv.sin_port = htons(8888);	//Define the port at which the server will listen for connections.
server.sin_addr.s_addr = INADDR_ANY;

fd = socket(AF_INET, SOCK_STREAM, 0);	//This will create a new socket and also return the identifier of the socket in fd
